...
namespace EBOS
{
    public partial class SinemaKontrol : UserControl
    {
        ...
        private void KartArayuzuOlustur()
        {
            ...
            txtArama.TextChanged += (_, __) => SinemaEtkinlikleriYukle(txtArama.Text);
        ...
        #region Veri & Kart
        private void SinemaEtkinlikleriYukle(string filtre = "")
        {
            flpKartlar.Controls.Clear();

            using var db = new AppDbContext();
            int kategoriId = db.EtkinlikTurleri
                               .Where(t => t.TurAdi.ToLower() == "sinema")
                               .Select(t => t.TurID)
                               .FirstOrDefault();
            ...
            if (etkinlikler.Count == 0)
            {
                ...
                {
                    Text = "Sinema etkinliği bulunamadı.",
                    AutoSize = true,
        ...
        private Control KartOlustur(Etkinlik e)
        {
            ...
            try { pb.Load(e.GorselYolu); } catch { /* yoksa boş kalsın */ }
            ...
            btnBilet.Click += (_, __) => new BiletAlForm(e.EtkinlikID, e.EtkinlikAdi, kullaniciEposta).ShowDialog();
