...
namespace EBOS
{
    public partial class SeminerKontrol : UserControl
    {
        ...
        private void KartArayuzuOlustur()
        {
            ...
            txtArama.TextChanged += (s, _) => SeminerEtkinlikleriYukle(txtArama.Text);
        ...
        #region Veri & Kart
        private void SeminerEtkinlikleriYukle(string filtre = "")
        {
            flpKartlar.Controls.Clear();

            using var db = new AppDbContext();
            int kategoriId = db.EtkinlikTurleri
                                .Where(t => t.TurAdi.ToLower() == "seminer")
                                .Select(t => t.TurID)
                                .FirstOrDefault();
            ...
            if (etkinlikler.Count == 0)
            {
                ...
                {
                    Text = "Seminer etkinliği bulunamadı.",
                    AutoSize = true,
        ...
        {
            // 🎴 180×340, Workshop/Tiyatro ile aynı stil ----------------------
